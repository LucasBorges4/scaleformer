...
class TimeFeatureEmbedding_new(nn.Module):
    def __init__(self, d_model, embed_type='timeF', freq='h'):
        ...
        freq_map = {'h': 4, 't': 5, 's': 6, 'm': 1, 'a': 1, 'w': 2, 'd': 3, 'b': 3}
        d_inp = freq_map[freq]
        # note: we add +1 for the scale indicator
        self.embed = nn.Linear(d_inp+1, d_model, bias=False)
        # removed cache self.concats to avoid device mismatch in DataParallel

    def forward(self, x, scale=1):
        # create concat_tensor ON THE SAME DEVICE as x to avoid device mismatch in DataParallel
        # use x.new_full to ensure dtype & device match
        val = 1.0/scale - 0.5
        concat_tensor = x.new_full((x.shape[0], x.shape[1], 1), val)
        x = torch.cat((x, concat_tensor), 2)
        return self.embed(x)
